def test_get_containers(db, master_node, ldap_container):
...
def test_get_containers_by_state(db, master_node, ldap_container):
    ldap_container.node_id = master_node.id
...
def test_count_containers(db, worker_node, ldap_container):
    ldap_container.node_id = worker_node.id
    db.persist(ldap_container, "containers")
    assert worker_node.count_containers(state=None) == 1


def test_count_containers_by_state(db, worker_node, ldap_container):
    ldap_container.node_id = worker_node.id
    ldap_container.state = "FAILED"
    db.persist(ldap_container, "containers")
